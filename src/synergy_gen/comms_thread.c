 ...
 **********************************************************************************************************************/
sf_el_ux_comms_instance_ctrl_t g_sf_comms0_instance_ctrl = { .p_cdc = NULL, };

/* Instance structure to use this module. */
const sf_comms_instance_t g_sf_comms0 = { .p_ctrl = &g_sf_comms0_instance_ctrl,
		.p_cfg = NULL, .p_api = &g_sf_el_ux_comms_on_sf_comms };
...
void comms_thread_create(void) {
	...
	err_g_cdc_queue = tx_queue_create(&g_cdc_queue, (CHAR *) "CDC Queue", 3,
			&queue_memory_g_cdc_queue, sizeof(queue_memory_g_cdc_queue));
	if (TX_SUCCESS != err_g_cdc_queue) {
		tx_startup_err_callback(&g_cdc_queue, 0);
	}

	UINT err;
	err = tx_thread_create(&comms_thread, (CHAR *) "Comms Thread",
			comms_thread_func, (ULONG) NULL, &comms_thread_stack, 1024, 2, 2, 1,
			TX_AUTO_START);
	if (TX_SUCCESS != err) {

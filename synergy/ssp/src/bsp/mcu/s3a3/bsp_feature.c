...
{
    p_flash_lp_feature->flash_clock_src = (uint8_t)CGC_SYSTEM_CLOCKS_FCLK; // S3A3 Flash uses FCLK
    p_flash_lp_feature->flash_cf_macros = 1U;     // S3A3 has 1 code flash HW macro and has a total of 512K Code Flash
    p_flash_lp_feature->cf_macro_size = 0x80000U; // S3A3 uses double access to access 512K as a single macro
}

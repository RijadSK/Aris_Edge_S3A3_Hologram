 ...
 * @defgroup BSP_MCU_MODULE_STOP_S3A3 Module Start and Stop
 *
...
/** Unit number corresponding to the DAC8 peripheral on this MCU. */
#define HW_DAC_UNIT_DAC8 (1U)
...
static uint32_t r_bsp_get_mstp_mask(ssp_feature_t const * const p_feature)
{
    ...
    if (data.bit.exception)
    {
        ...
        switch (p_feature->id)
        {
            ...
            case SSP_IP_DAC:
                if (p_feature->unit == HW_DAC_UNIT_DAC8)
                {
                    mask = (1U << 19);
                }
...
#endif /* if defined(BSP_MCU_GROUP_S3A3) */


/** @} (end defgroup BSP_MCU_MODULE_STOP_S3A3) */

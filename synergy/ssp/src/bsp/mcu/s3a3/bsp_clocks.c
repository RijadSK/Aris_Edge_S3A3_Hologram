 ...
 * @defgroup BSP_MCU_CLOCKS_S3A3 Clock Initialization
 *
...
#define USBCKCR_POINTER             (0x4001E0D0U)
...
void bsp_clock_init (void)
{
    ...
    volatile uint8_t * const p_usbckcr = (volatile uint8_t * const) USBCKCR_POINTER;
    * p_usbckcr = (* p_usbckcr) | (1 << 0);
...
ssp_err_t bsp_clock_set_callback(bsp_clock_set_callback_args_t * p_args)
{
    ...
    if (BSP_CLOCK_SET_EVENT_PRE_CHANGE == p_args->event)
    {
         ...
         * S3A3 does not require SRAM wait state control. In fact the register is unimplemented for the S3A3.
         */
    ...
    if (BSP_CLOCK_SET_EVENT_POST_CHANGE == p_args->event)
    {
        ...
        {
            bsp_clocks_mem_wait_set(CGC_ZERO_WAIT_CYCLES);
        }
    }
    return SSP_SUCCESS;
}
...
/** @} (end defgroup BSP_MCU_CLOCKS_S3A3) */
